...
import { checkAiContent } from "../safety/content-policy.ts";
...
function text(value: unknown, limit: number): string {
  ...
  const decision = checkAiContent(value);
  if (!decision.ok) throw new Error("UNSAFE_AI_OUTPUT");
  if (charCount(decision.text) > limit) throw new Error("AI_OUTPUT_TOO_LONG");
