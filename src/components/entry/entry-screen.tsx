...
import { MatchmakingPanel } from "./matchmaking-panel.tsx";
...
export function EntryScreen({ session, theme }: { session: GameSession; theme?: GameTheme }) {
  ...
  return <main className="entry-hero" ref={root}>
    ...
    <nav className="entry-nav"><div className="entry-nav-brand"><span>◒</span><strong>人味圆桌局</strong></div><div className="entry-links"><a href="#enter">入席</a><a href="#enter" onClick={() => session.setMode("spectator")}>观战</a><button type="button" className="nav-rules-button" aria-haspopup="dialog" onClick={() => setRulesOpen(true)}><HelpCircle size={18} />规则</button></div><div className="entry-nav-status"><i className={session.connected ? "is-online" : ""} />{session.connected ? "服务在线" : "连接中"}</div></nav>
    ...
    <div className="entry-workspace">
      {session.matchmaking && <MatchmakingPanel controls={session.matchmaking} disabled={!session.connected || session.busy} />}
      ...
      <div className="entry-actions"><button className="primary" disabled={!session.connected || session.busy || session.matchmaking?.waiting || session.matchmaking?.busy} onClick={session.join}>进入房间<ArrowRight size={18} /></button><button className="secondary" disabled={!session.connected || session.busy || session.matchmaking?.waiting || session.matchmaking?.busy} onClick={session.create}>创建新房</button></div>
