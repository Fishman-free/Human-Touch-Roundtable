...
export class SqlitePersistence implements RoomStore, SessionStore {
  ...
  constructor(path: string) {
    ...
    else if (version !== 1) { this.db.close(); throw new Error("UNSUPPORTED_SCHEMA_VERSION"); }
  }
  ...
  async create(record: SessionRecord): Promise<boolean> {
    ...
    const result = this.db.prepare(`
      INSERT OR IGNORE INTO sessions(id, room_id, viewer_json, token_hash, created_at)
      VALUES (?, ?, ?, ?, ?)
  ...
  async find(id: string): Promise<SessionRecord | null> {
    const row = this.db.prepare(`
      SELECT id, room_id, viewer_json, token_hash, created_at FROM sessions WHERE id = ?
