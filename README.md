...
- 开发专用会话密钥。生产启动必须显式设置`SESSION_HMAC_KEY`。

生产检查与启动：

```sh
npm run build
SESSION_HMAC_KEY="至少32字节的稳定密钥" npm start
```
...
Socket层签发和验证会话凭据，再把认证身份交给运行时；SQLite可以恢复完整私有房间和会话摘要。浏览器版本已经可以用本地题目和模拟AI走完整流程。当前仍不包含真实知乎/模型请求，本地题目中的知乎链接不代表已经完成接口接入。

本轮优先建立协作边界并完善后端，不新增前端产品功能。后续重点为真实外部适配器、恢复与并发边界、浏览器请求重试及公网运行准备。不要把开发模拟器当作已经完成真实API接入。
