...
当前真实知乎网关尚未实现，默认生产题目模式会拒绝启动；公开部署前先完成该阻塞项。完整环境变量和部署步骤见[生产部署](docs/operations/deployment.md)。
...
Socket层签发和验证会话凭据，再把认证身份交给运行时；SQLite可以主动恢复完整私有房间和会话摘要。浏览器版本可用本地题目和模拟AI走完整流程；LLM网关已具备DeepSeek/GLM接入和主备切换，但尚未用项目密钥在线验收。真实知乎网关仍未实现，本地题目链接不代表已完成接口接入。

后续重点为知乎官方网关、真实模型契约测试、浏览器请求重试、真实代理压测和备份恢复演练。不要把开发模拟器或生产基础设施冒烟当作真实API/公网验收。
