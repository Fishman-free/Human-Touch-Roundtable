...
import { candidateTopicPacks } from "../src/topics/static-topic-provider.ts";
...
const provider = candidates
  ? new VerifiedTopicProvider(candidateTopicPacks, new ZhihuSearchQuestionGateway(new ZhihuContentClient({
...
const candidateIds = all || candidates ? provider.candidateIds : [candidate ?? provider.candidateIds[0]];
