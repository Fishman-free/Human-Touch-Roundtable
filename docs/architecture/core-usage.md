...
> 2026-09-10：核心、运行时、网络会话和SQLite持久化已实现。真实知乎与模型供应商尚未接入。
...
观战在核心中是只读视图，不占座位或触发准备条件。陌生participantId不能操作已有座位，合法重连使用原participantId即可定位身份；凭据签发与验证待会话层实现。
...
此验证不代表联网、SQLite恢复、浏览器体验或真实AI质量已经验收。
