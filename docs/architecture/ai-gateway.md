...
- 请求发出前，AI 座位先等一段自己的随机节拍（`AI_BEAT_MS`，3～10 秒）：一桌人同时秒回是最明显的机器特征，而模型本身只要几百毫秒。节拍不侵占模型预算，`aiBeatMs()` 把它压在“阶段剩余时间减去模型预算”之内，辩论中 20 秒的指认阶段因此只等 0～5 秒。等待期间上下文在请求发出时才读取，晚发言的座位能看到这期间其他人的话。
