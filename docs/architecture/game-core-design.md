...
> 日期：2026-09-10
> 状态：游戏核心、房间运行时、Socket会话、SQLite持久化和浏览器页面已实现；真实外部服务仍为设计方案。实现说明见[核心调用约定](core-usage.md)、[房间运行时](runtime-usage.md)、[Socket协议](socket-protocol.md)、[SQLite持久化](sqlite-persistence.md)和[浏览器应用](web-app.md)。
...
stateDiagram-v2
    ...
    answering1 --> answering2: 全部提交或90秒到期
    answering2 --> answering3: 全部提交或90秒到期
    answering3 --> debating: 全部提交或90秒到期
...
| answering | 每座位提交一次答案 | 设置90秒截止时间，启动AI任务 | 全员完成或超时补全 |
...
出局集合 = 有效票的目标集合（去重）
若所有AI座位都在出局集合：普通人胜
否则若所有影子座位都不在出局集合：影子胜
否则：AI胜
```

即使某普通人被其他普通人指向，其票仍参与本次结算。所有出局同时产生，无先后淘汰。两个影子中任意一个出局都会使影子方失去获胜条件，依据规则书第6节执行。

数量相等意味着普通人胜利时，每张普通票必定命中不同AI，不可能同时投出影子。规则书的“即使同时有影子出局”在当前人数规则下不可达。

## 5. 客户端事件协议

所有写动作携带 `commandId`；对局内动作另带 `matchId` 和 `phaseToken`。服务端从认证会话识别发起者，不接受客户端声明自己是哪个座位或角色。

| 客户端事件 | 参数要点 | 限制 |
|---|---|---|
| room:create | commandId | 创建候场房间 |
| room:join | roomId、mode:player/spectator | player仅限候场且真人不超过5 |
| room:ready | ready:boolean | 仅候场真人 |
| room:resume | 重连凭据 | 恢复原座位或观战身份 |
| game:answer | round、text、stance? | 当前轮且尚未提交 |
| game:accuse | targetSeatId、text | 当前指认者，目标非自己 |
| game:respond | text | 当前被指认者 |
| game:followup | text | 当前指认者 |
| game:skip-followup | 无业务参数 | 当前指认者，可选追问 |
| game:vote | targetSeatId | 仅投票阶段，非自己、未投过 |
| room:sync | lastRevision | 重连或发现状态缺失时拉取快照 |

服务端发送 `room:state`：经过接收者权限投影的完整快照，含 `revision`、`serverNow`、截止时间、可执行动作、公开记录和本人私有身份。P0最多8座位，优先使用完整快照减少增量合并复杂度。

动作回执为成功（commandId、revision）或失败（commandId、错误码、面向用户的提示）。错误码包括阶段错误、权限错误、重复动作、非法目标、超长等。规则操作错误需要提示；外部AI/题目服务错误不能直接展示。
...
- AI知道全部身份，但提示词要求通过匿名发言和行为游戏，不直接宣告服务端身份表；具体生成质量留到AI实现阶段验证。
...
重启后依据保存的绝对截止时间补做超时动作，每次推进持久化一次；对尚未过期的阶段重新安排剩余时间。座位永久保留在该局记录中；过期房间清理和凭据保留期限需后续制定，不能实现成断线即回收。

## 7. 题目与默认答案

保留规则书附录B作为片单占位格式。选择题目时必须校验：知乎问题标识、标题、原文链接、用于第三轮的最高赞回答材料、三轮默认答案池。

知乎接口是否直接返回高赞回答与共识尚未核实；最高赞回答材料的取得方式是集成前置检查，不能仅获取标题就假定三轮资料齐全。共识摘要可预制，但必须对应锁定的问题。
...
src/
  app/                            # 首页、房间路由、健康检查
  components/room/                # 答题流、座位、辩论、投票、结算
  client/                         # Socket连接、重连、useRoom
  ui/                             # 主题和展示视图模型
  game/
    model.ts                      # 私有权威模型
    commands.ts                   # 游戏动作
  ...
  server/                         # 会话认证、Socket处理、依赖组装
  topics/                         # 知乎接口、片单、材料校验
  ai/                             # 上下文、生成任务、过滤与默认内容
  repository/                     # 内存和SQLite实现
  poster/                         # 后续结果导出
prisma/schema.prisma              # 选择Prisma时唯一的schema位置
data/topic-packs/                  # 经人工核对的预制题目包
tests/                            # 核心规则、协议和端到端验收
```
...
| 顺序 | 工作 | 完成标准 |
|---|---|---|
| 1 | 核心模型、转换、结算、视图裁剪 | 无网络环境完成3/5/6/8座位整局 |
| 2 | 虚拟时钟、房间队列、默认内容 | 截止边界、提前结束、迟到AI均确定可复现 |
| 3 | 会话与Socket协议 | 多客户端同步，重连原座，观战不可操作 |
| 4 | 完整房间UI | 手机可完成准备、回答、辩论、投票、查看日志 |
| 5 | SQLite保存与恢复 | 重启仍能恢复私有身份及整局记录 |
| 6 | 知乎与模型接入 | 成功取题才开局，失败不泄露身份或服务信息 |
...
- 普通人投中各个不同AI则普通人胜；重复命中且影子全存活则影子胜。
- 普通人未全歼AI且任意影子出局则AI胜；影子/AI干扰票不产生出局。
- 被普通票指向的普通人，其已投出的票仍参与同时结算。
- 缺失投票记录为未投票；无普通人命中影子且普通人未胜时，影子胜。
...
已按冻结规则确定的边界：影子胜利要求全部影子存活；8分钟辩论上限优先；出局统一在投票结束后计算。

本文补充但尚未作为产品规则冻结的建议：随机匿名座位、指认超时目标选择、辩论默认回应、辩论文本长度、全部题目失败后的等待与退避策略。其余模块设计可独立推进，不需要为了这些边界停下整个设计阶段。
