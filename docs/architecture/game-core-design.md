...
- AI知道全部身份；结构化Prompt、启发式泄露过滤和主备网关已实现，但真实模型行为与内容安全仍需带密钥验收。
...
规则书附录B已正式化为TopicPack v1。选择题目时校验知乎问题标识、标题、原文链接、第三轮最高赞回答材料、三轮默认答案池和来源元数据。
...
| 待在线验收 | DeepSeek/GLM真实调用 | 项目密钥契约测试 |
| 当前阻塞 | 知乎官方网关实现 | Access Secret、端点及字段确认 |
| 待人工验收 | 公网代理、多人长局、移动端、恢复演练 | 部署环境测试 |
