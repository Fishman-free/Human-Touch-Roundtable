...
| 能力 | 当前证据 | 仍需完成 |
|---|---|---|
| 知乎 | 客户端和网关已实现；11个候选中9个单次在线核验；问题推荐与回答摘要接口已实现并有模拟测试 | 2题核验、动态题模型生成共识与默认答案的质量和内容安全、持久缓存、长期额度与稳定性 |
| 模型 | GLM和DeepSeek七动作单次真实验证；failover自动测试 | 长期错误率、成本、容量和真实故障注入 |
| 内容安全 | 确定性隐私/凭据/注入/身份过滤 | 完整审核服务、人工流程和对抗测试 |
| 命令可靠性 | 服务端回执幂等和浏览器Outbox自动测试 | 真实浏览器弱网/休眠E2E |
| 恢复 | SIGKILL、WAL、连续截止和在线备份自动测试 | 磁盘写满、断电、网络存储和异地恢复 |
| 代理与负载 | 本地production mode的40并发观战房间冒烟 | 真实域名、Caddy/CDN、完整对局与资源容量 |
| 管理接口 | 自动测试与生产进程冒烟 | 必须由网络层隔离；尚无角色权限和管理审计 |

管理员API是内部运维接口，不是公网管理API。未配置Token时返回404只是在协议层隐藏路由，不是网络隔离；Token泄露可导致房间删除或会话撤销。公网部署必须用私有网络、防火墙或反向代理限制`/api/admin/*`，并建立Token轮换与审计。

完整证据等级见[验证状态矩阵](validation-status.md)。应用内保护不能替代CDN、入口网关或完整内容审核。
